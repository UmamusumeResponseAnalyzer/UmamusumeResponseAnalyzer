...
* `/plugin` 或 `/plugin list` 列出插件运行期状态，`/plugin load|unload|reload <InternalName>` 只改变当前进程内加载状态，不安装、不删除插件文件、不写禁用配置。命令结果中的非空 message 写入全局最近日志；只有 message 而没有 display 时同时显示 global notification，有 display 时通过 popup 呈现。
...
* 下载先写临时文件，核对 manifest 的 Author、InternalName、Version 和 ZIP 包契约后替换目标 ZIP，上限 64 MiB。下载或校验失败保留现有 ZIP。安装和更新保存插件包，重启 URA 后生效。
...
* attribute 和程序化 analyzer handler 都返回 `ValueTask`。程序化 analyzer 注册只能在 `Initialize` 或宿主调用的 `OnStarted` 回调中发生。`Initialize` 中的 analyzer 与 background 注册在共享组全部成员初始化成功后一起生效；`OnStarted` 中的注册在该回调成功后生效。失败时不留下该阶段的部分注册。
...
* 运行中卸载插件时，宿主先停止接收该 generation 的新回调，取消 background operation，等待已准入回调与后台任务全部结束，再调用 `Dispose()` 并清理 analyzer、事件订阅及快捷键归属。一次 packet 分发为每个匹配插件持有一个 generation lease，直到整轮分发完成，因此卸载会等待尚未执行到的同轮 handler；配置 dialog 也持有卸载等待的 lease。插件应响应后台 cancellation，并在 `Dispose()` 中清理自己创建的 panel 和资源；宿主不会因卸载插件而删除全局共享 workspace。插件回调内发起加载、卸载或重载，以及并发生命周期事务，均立即抛出 `InvalidOperationException`。`Initialize` 或 `OnStarted` 回调抛异常时不会提交该回调暂存的 analyzer/background 注册。
