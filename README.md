...
* `/plugin` 或 `/plugin list` 列出启动时确定的插件及其当前运行状态、错误原因。命令结果中的非空 message 写入全局最近日志；只有 message 而没有 display 时同时显示 global notification，有 display 时通过 popup 呈现。
* 插件更新 workspace panel 时默认会切到该 workspace；bootstrap 刷新和异常写入不会切换当前 workspace。按 `Ctrl+B` 返回启动 workspace，按 `P` 查看已加载插件列表，按 `Ctrl+C` 退出程序。退出不保证正在执行的请求处理、Host 命令或插件后台任务完成。
...
* 安装成功返回 `{ok: true, installed: "<InternalName>", error: null}`，失败返回 `ok: false` 和错误原因。成功只表示 ZIP 已保存，网页和 Host 提示重启后生效。`/uracloud/status` 返回 Host 版本和当前已加载插件，其中 `loaded: true, source: null`；网页会显示来源未知，不能标记已安装同一 Release。
...
* `manifest.json` 必须精确包含 `Author`、`InternalName`、`DisplayName`、`Description`、`Changelog`、`Version`、`Dependencies`、`Targets`、`RepositoryUrl`、`LastUpdate`、`Category`、`Homepage`。`Dependencies` 声明直接引用的插件 internal name，用于已安装包的程序集共享及初始化顺序；缺失项不阻止加载，必需程序集实际缺失时按下述入口规则处理。软依赖指插件自己检查目标是否安装并保护可选调用的情况，manifest 不另作分类。插件集合、代码和依赖组在启动时确定，共享 load context 保留至进程退出。已安装图中的自依赖、重复名称和依赖环使插件加载失败；共享组内的程序集名称大小写冲突或同名程序集内容冲突使该组加载失败，无关组继续加载。`IPluginContext.IsPluginAvailable` 检查声明目标是否在启动时已安装且符合目标筛选，不反映运行期故障状态；可选插件类型不得出现在 Consumer 导出类型签名或启动必经字段初始化中。`Targets` 为空、命中配置的 repository targets，或配置未设置 targets 时，宿主才创建插件实例并使其进入运行期；否则 ZIP 保持安装但插件不初始化。
...
* attribute 和程序化 analyzer handler 都返回 `ValueTask`。程序化 analyzer、`OnStarted` 订阅和 background 注册只能在有效的 `Initialize` 或宿主调用的 `OnStarted` 回调阶段内串行进行；阶段结束后，捕获该阶段的异步上下文也不能继续注册。每个插件的 analyzer 与 background 注册在该插件的 `Initialize` 成功后生效；`OnStarted` 回调中的 analyzer 与 background 注册在该回调成功后生效。失败时不提交该阶段暂存的 analyzer/background 注册。`OnStarted` 订阅直接生效，不参与该提交。
...
* 插件必须实现 `Initialize(IPluginContext context)`；`Dispose()` 和 `ConfigPromptAsync(...)` 有默认空实现。
...
* `context.Events.OnStarted(Func<CancellationToken, ValueTask>)` 用于订阅宿主启动事件；`context.RunBackground(Func<CancellationToken, ValueTask>)` 把插件长期任务交给宿主管理。
* analyzer 遇到必需程序集缺失时，宿主报告插件、注册来源、endpoint、执行阶段和缺失程序集，并仅将出错的单条 analyzer 注册停用至进程退出。该注册已开始的调用自然结束，后续调用跳过该注册；同插件的其他注册、同一 handler 在其他 endpoint 的注册继续各自执行。此停用不等待在途调用、不取消后台任务、不调用 `Dispose()`，也不移除注册。
* `ConfigPromptAsync`、`OnStarted`、快捷键和宿主管理的后台任务遇到必需程序集缺失时，宿主将该插件停用至进程退出：停止准入新回调，取消后台任务，等待已准入回调与后台任务结束，再调用 `Dispose()` 并清理 analyzer、事件订阅及快捷键归属。插件应响应后台 cancellation，并在 `Dispose()` 中清理自己创建的 panel 和资源；宿主保留程序集和全局共享 workspace。普通业务异常按原入口报告；插件停用不连带停用其他插件。
