...
namespace UmamusumeResponseAnalyzer.Game
{
    ...
    public class TurnStats
    {
        ...
        public bool isEffect102;//是否为女神情热状态
        ...
        //三女神等级
        public int yellowVenusLevel;
        public int redVenusLevel;
        public int blueVenusLevel;

        public int venusStat1_venusTrain;//女神在哪个训练
        public bool venusStat1_isVenusCountConcerned;//非情热，且无蓝女神
        public bool venusStat1_venusEvent;//成功召唤出女神三选一事件
        public TurnStats() {
            isTraining = false;
            motivation = 0;
            playerChoice = -1;
            ...
            redVenusLevel=0;
            blueVenusLevel=0;
            venusStat1_venusTrain = -100;
            venusStat1_isVenusCountConcerned = true;
            venusStat1_venusEvent = false;
        }
    }
        ...
        public static void print()
        {
            //统计召唤女神次数与来的次数
            {
                ...
                for (int turn = 1; turn < currentTurn; turn++)//不考虑当前回合，因为当前回合还未选训练
                {
                    ...
                    if (turnStat.isTraining && turnStat.venusStat1_isVenusCountConcerned && turnStat.venusStat1_venusTrain == turnStat.playerChoice)
                    {

                        concernedTurnTotal += 1;
                        if (turnStat.venusStat1_venusEvent)
                        {
                            venusEventCount += 1;
                        }
                    }
            ...
            //统计掉心情次数
            {
                ...
                AnsiConsole.MarkupLine($"这局掉了[yellow]{motDropCount}[/]次心情（忽略刚掉就开红女神的情况）");
            }
            ...
            //统计训练属性pt收益
            {
                ...
                for (int turn = currentTurn-1; turn >= 1; turn--)
                {
                    ...
                    double venusBonus = venusLevelBonus[stats[turn].blueVenusLevel] + venusLevelBonus[stats[turn].yellowVenusLevel] + venusLevelBonus[stats[turn].redVenusLevel];
